...
def create_parameters(dt=0.001):
    ...
    p = {'network_size': 200,              # Number of units in network
...
class differentialRNN:
    ...
    def train(self,inps_and_targs, monitor_training=0, **kwargs):
        ...
        DRNN = differentialRNN(hyperparameters = self.p, num_inputs = D_num_total_inps, num_outputs = 1)
    ...
    def test(self, inps_and_targs, **kwargs):
        ...
        ax.set_title('d-RNN Testing: Wait')
        ...
        for idx in range(p['test_trials']):
            ...
            ax.set_title('Sparsity learning trajectory')
            test_fig.canvas.draw()
